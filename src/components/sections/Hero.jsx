...
const Hero = () => {
 ...
 const [dashboardTab, setDashboardTab] = useState('webstore');
 ...
 const dashboardMenu = [
 { id: 'orders', icon: ShoppingBag, label: t('Commandes') },
 { id: 'analytics', icon: TrendingUp, label: t('Statistiques') },
 { id: 'customers', icon: Users, label: t('Clients') },
 { id: 'finance', icon: DollarSign, label: t('Finance') },
 { id: 'webstore', icon: Globe, label: t('Boutique') },
 ];
 ...
 {/* Title */}
 <h1
 className="text-5xl lg:text-[56px] font-extrabold text-slate-900 dark:text-white mb-8 leading-[1.05] animate-slide-up tracking-tight"
 ...
 <span className="text-xs font-bold uppercase tracking-wider">{t("7 jours d'essai")}</span>
 </div>
 <div className="flex items-center gap-2 text-slate-400 dark:text-slate-500">
 <ShieldCheck size={16} className="text-primary" />
 <span className="text-xs font-bold uppercase tracking-wider">{t("Sans engagement")}</span>
 </div>
 <div className="flex items-center gap-2 text-slate-400 dark:text-slate-500">
 <Zap size={16} className="text-primary" />
 <span className="text-xs font-bold uppercase tracking-wider">{t("Mise en route rapide")}</span>
 </div>
 <div className="flex items-center gap-2 text-slate-400 dark:text-slate-500">
 <Rocket size={16} className="text-primary" />
 <span className="text-xs font-bold uppercase tracking-wider">{t("Conçu pour l’e-commerce moderne")}</span>
 </div>
 </div>
 </div>
 ...
 <p className="text-[10px] font-black text-primary uppercase leading-tight">{t("Nouvelle Vente !")}</p>
 <p className="text-xs font-bold text-slate-900 dark:text-white whitespace-nowrap">{lastSale.item} - {lastSale.city}</p>
 </div>
 <p className="text-sm font-black text-primary ms-2">{lastSale.amount} DA</p>
 </div>
 )}
 ...
 <div className="w-14 xs:w-20 border-e border-slate-100 dark:border-slate-800 py-6 flex flex-col items-center gap-6 bg-slate-50/50 dark:bg-slate-900/50">
 ...
 <span className="font-black text-lg italic leading-none">{t("R")}</span>
 </div>
 ...
 className={`p-2 xs:p-2.5 rounded-xl transition-all cursor-pointer relative group/icon ${item.id === 'webstore' ? 'mt-auto' : ''} ${dashboardTab === item.id ? 'bg-primary/10 text-primary shadow-sm' : 'text-slate-400 dark:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800'}`}
 >
 ...
 {[t('En attente'), t('Confirmé'), t('Expédié')].map((s, i) => (
 ...
 {[
 { id: '#7842', name: 'Karim S.', city: 'Alger', status: t('Confirmé'), color: 'bg-green-500' },
 { id: '#7841', name: 'Mouna L.', city: 'Oran', status: t('En attente'), color: 'bg-amber-500' },
 ].map((order, i) => (
 ...
 <p className="text-[8px] xs:text-[10px] font-black text-slate-900 dark:text-white">{t("4,500 DA")}</p>
 </div>
 </div>
 ))}
 </div>
 ...
 <p className="text-[8px] xs:text-[10px] font-black text-primary uppercase tracking-widest mb-4">{t("Revenus Mensuels")}</p>
 ...
 <p className="text-[8px] xs:text-[10px] font-black uppercase tracking-widest opacity-60 mb-1">{t("Solde Actuel")}</p>
 <p className="text-xl xs:text-3xl font-black tabular-nums tracking-tighter">45,620 <span className="text-xs">{t("DA")}</span></p>
 <div className="flex gap-2 mt-6 xs:mt-8">
 <button className="flex-1 py-2 xs:py-3 bg-white text-primary text-[8px] xs:text-[10px] font-black uppercase rounded-lg xs:rounded-xl font-black">{t("Retrait")}</button>
 </div>
 </div>
 </div>
 )}
 ...
 className={`p-2 xs:p-4 rounded-xl xs:rounded-2xl transition-all border-2 flex xs:flex-col items-center justify-center gap-2 xs:gap-3 flex-1 xs:flex-none whitespace-nowrap min-w-[80px] xs:min-w-[100px] ${builderSubTab === sub ? 'border-primary bg-white dark:bg-slate-900 shadow-sm' : 'border-transparent bg-slate-50 dark:bg-slate-900/50'}`}
 >
 {sub === 'sections' && <Layout size={16} className={builderSubTab === sub ? 'text-primary' : 'text-slate-400'} />}
 {sub === 'styles' && <Palette size={16} className={builderSubTab === sub ? 'text-primary' : 'text-slate-400'} />}
 {sub === 'polices' && <Type size={16} className={builderSubTab === sub ? 'text-primary' : 'text-slate-400'} />}
 <p className={`text-[9px] xs:text-[10px] font-black uppercase tracking-tight ${builderSubTab === sub ? 'text-slate-900 dark:text-white' : 'text-slate-400'}`}>{sub}</p>
 </button>
 ))}
 </div>

 <div className="flex-1 bg-slate-50 dark:bg-slate-900 rounded-2xl xs:rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-800 p-4 flex flex-col relative overflow-hidden animate-fade-in" key={builderSubTab}>

 <div className="flex-1 overflow-y-auto no-scrollbar scroll-smooth flex flex-col gap-3 min-h-0 w-full relative z-0 pb-16">
 {builderSubTab === 'sections' && (
 <div className="w-full flex flex-col gap-3">
 {/* Header */}
 <div className="group relative border border-transparent hover:border-primary/20 rounded-xl p-2 transition-all cursor-move bg-white dark:bg-slate-950 shadow-sm hover:shadow-md">
 ...
 {/* Hero */}
 <div className="group relative border border-transparent hover:border-primary/20 rounded-xl p-2 transition-all cursor-move bg-white dark:bg-slate-950 shadow-sm hover:shadow-md ring-1 ring-slate-100 dark:ring-slate-800/50">
 <div className="h-32 bg-slate-50 dark:bg-slate-900 rounded-lg flex flex-col items-center justify-center gap-2.5 relative overflow-hidden">
 {/* blobs */}
 <div className="absolute top-0 right-0 w-20 h-20 bg-blue-400/10 rounded-full blur-xl translate-x-1/2 -translate-y-1/2" />
 <div className="absolute bottom-0 left-0 w-20 h-20 bg-primary/10 rounded-full blur-xl -translate-x-1/2 translate-y-1/2" />

 <div className="w-32 h-3 rounded-full bg-slate-200 dark:bg-slate-700" />
 <div className="w-20 h-2 rounded-full bg-slate-200 dark:bg-slate-700/60" />
 <div className="w-14 h-5 mt-2 rounded-md text-[6px] bg-primary flex items-center justify-center text-white font-black shadow-sm">
 {t("ACHETER")}
 </div>
 </div>
 </div>
 ...
 {[1, 2].map(i => (
 <div key={i} className="flex flex-col gap-2 bg-slate-50 dark:bg-slate-900 p-2 rounded-lg border border-slate-100 dark:border-slate-800">
 <div className="h-20 bg-white dark:bg-slate-950 rounded flex items-center justify-center">
 <Package size={12} className="text-slate-200 dark:text-slate-800" />
 </div>
 <div className="w-full h-2 rounded-full bg-slate-200 dark:bg-slate-700 mt-1" />
 <div className="flex justify-between items-center">
 <div className="w-1/2 h-2 rounded-full bg-primary/40" />
 ...
 {builderSubTab === 'styles' && (
 <div className="flex-1 flex flex-col items-center justify-center w-full h-full text-slate-300 dark:text-slate-600 min-h-[150px]">
 <Palette size={24} className="mb-2 opacity-50" />
 <p className="text-[10px] font-black uppercase tracking-widest opacity-50">{t("Couleurs du thème")}</p>
 <div className="flex gap-2 mt-4">
 <div className="w-6 h-6 rounded-full bg-blue-500 shadow-sm border border-white" />
 <div className="w-6 h-6 rounded-full bg-indigo-500 shadow-sm border border-white opacity-50 scale-75" />
 <div className="w-6 h-6 rounded-full bg-emerald-500 shadow-sm border border-white opacity-50 scale-75" />
 </div>
 </div>
 )}

 {builderSubTab === 'polices' && (
 <div className="flex-1 flex flex-col items-center justify-center w-full h-full text-slate-300 dark:text-slate-600 min-h-[150px]">
 <Type size={24} className="mb-2 opacity-50" />
 <p className="text-[10px] font-black uppercase tracking-widest opacity-50">{t("Typographie globale")}</p>
 <div className="flex flex-col gap-2 mt-4 items-center">
 <div className="text-xs font-serif font-black text-slate-900 dark:text-white border border-slate-200 dark:border-slate-800 px-3 py-1 rounded bg-white dark:bg-slate-950">{t("Playfair Display")}</div>
 <div className="text-xs font-sans text-slate-400">{t("Inter")}</div>
 </div>
 </div>
 )}
 </div>

 <div className="absolute bottom-0 left-0 right-0 p-4 bg-slate-50 dark:bg-slate-900 z-10">
 <button className="w-full py-3 bg-primary text-white text-[10px] xs:text-xs font-black uppercase rounded-2xl hover:bg-primary/90 transition-colors shadow-lg shadow-primary/20">{t("Enregistrer")}</button>
 </div>
 </div>
 </div>
 )}
 ...
 <p className="text-xl font-black text-slate-900 dark:text-white tabular-nums tracking-tighter">{revenue.toLocaleString()} <span className="text-[10px]">{t("DA")}</span></p>
 </div>
 ...
 >
 {isProcessing ? <Loader2 size={16} className="animate-spin" /> : t('Prendre Commande')}
 </button>
 </div>
 </div>
 </div>
