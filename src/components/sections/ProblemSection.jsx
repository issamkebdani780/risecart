...
const ProblemSection = () => {
 ...
 const beforeItems = [
 { text: t('Commandes mal suivies (Excel)'), icon: <FileWarning size={16} /> },
 { text: t('Tunnel de vente fragile'), icon: <MousePointerClick size={16} /> },
 { text: t('Manque total de clarté'), icon: <Eye size={16} className="opacity-40" /> },
 { text: t('Perte de temps colossale'), icon: <Clock size={16} /> },
 { text: t('Design peu professionnel'), icon: <LayoutGrid size={16} /> },
 { text: t('Outils dispersés & complexes'), icon: <AlertTriangle size={16} /> }
 ];

 const afterItems = [
 { text: t('Boutique claire & moderne'), icon: <Sparkles size={20} /> },
 { text: t('Gestion ultra centralisée'), icon: <ShieldCheck size={20} /> },
 { text: t('Vision précise des chiffres'), icon: <BarChart3 size={20} /> },
 { text: t('Meilleure organisation'), icon: <Target size={20} /> },
 { text: t('Véritable montée en gamme'), icon: <Rocket size={20} /> },
 { text: t('Système professionnel robuste'), icon: <Zap size={20} /> }
 ];
 ...
 <AlertTriangle size={12} /> {t("Diagnostic Performance")}
 </div>
 <h2 className="text-4xl lg:text-7xl font-extrabold text-slate-900 dark:text-white mb-8 leading-[1.1] tracking-tight">
 {t("Arrête de bricoler, commence à")} <span className="text-primary italic">{t("scaler")}</span>.
 </h2>
 <p className="text-slate-500 dark:text-slate-400 text-lg lg:text-xl font-medium max-w-2xl mx-auto leading-relaxed">
 {t("La plupart des e-commerçants s'épuisent avec des outils déconnectés. Voici comment RiseCart transforme votre chaos en un système professionnel.")}
 </p>
 </div>
 ...
 <div className="absolute top-8 right-8 text-[8px] font-black text-slate-300 dark:text-slate-700 uppercase tracking-[0.3em]">{t("Phase de stagnation")}</div>
 
 <div className="flex items-center mb-10">
 <h3 className="text-3xl font-black text-red-500/80 flex items-center gap-4 tracking-tighter italic">
 {t("Avant")} <XCircle className="group-hover:rotate-12 transition-transform" size={28} />
 </h3>
 </div>
 
 <div className="space-y-6">
 <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-6">{t("Le chaos classique :")}</p>
 ...
 <div className="absolute top-8 right-12 text-[8px] font-black text-primary uppercase tracking-[0.3em] z-10 animate-pulse">{t("Système Actif")}</div>
 ...
 {t("Après")} <ShieldCheck className="group-hover:scale-110 transition-transform" size={32} strokeWidth={2.5} />
 </h3>
 </div>

 <div className="space-y-6 relative z-10">
 <p className="text-xs font-black text-primary uppercase tracking-widest mb-6">{t("La transformation RiseCart :")}</p>
 <ul className="grid gap-4">
 ...
 <div>
 <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{t("Résultat")}</p>
 <p className="text-lg font-black text-slate-900 dark:text-white">{t("Croissance Structurée")}</p>
 </div>
 ...
 {t("Scaler maintenant")} <ArrowRight size={16} />
 </button>
 </div> */}
 </div>
 </div>
