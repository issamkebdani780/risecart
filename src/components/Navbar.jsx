  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-50 pt-4 px-4 sm:px-6 pointer-events-none">
        <div 
          className={`container mx-auto px-4 lg:px-6 py-2.5 bg-white dark:bg-slate-900 rounded-[24px] border border-slate-100 dark:border-slate-800 shadow-sm transition-all duration-300 flex items-center justify-between pointer-events-auto ${
          ...
          <nav className="hidden xl:flex items-center gap-6">
            {navLinks.map((link) => (
              <Link 
                key={link.path}
                to={link.path}
                className={`text-[13px] font-bold transition-all hover:text-primary ${
                  location.pathname === link.path ? 'text-primary' : 'text-slate-400 dark:text-slate-500'
                }`}
              >
                {link.name}
              </Link>
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-xl rounded-2xl py-2 flex flex-col min-w-[120px]">
                  {languages.map((lng) => (
                    <button 
                      ...
                      className={`px-4 py-2.5 text-xs font-bold text-left rtl:text-right w-full transition-colors flex items-center justify-between ${i18n.language?.startsWith(lng.code) ? 'text-primary bg-blue-50/50 dark:bg-blue-900/20' : 'text-slate-500 hover:text-primary dark:hover:text-primary hover:bg-slate-50 dark:hover:bg-slate-800'}`}
                    >
                      {lng.name}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-white dark:bg-slate-900 rounded-[32px] p-8 shadow-2xl border border-slate-100 dark:border-slate-800 animate-fade-in text-center">
            ...
            <Link to="/" onClick={() => setIsOpen(false)} className="flex flex-col items-center gap-3 mb-10">
              ...
              />
              <span className="text-xl font-black text-slate-900 dark:text-white tracking-tight">RiseCart</span>
            </Link>
              ...
              {navLinks.map((link) => (
                <Link 
                  ...
                  className={`text-sm font-bold transition-all ${
                    location.pathname === link.path ? 'text-primary' : 'text-slate-500 hover:text-primary dark:text-slate-400'
                  }`}
                >
                  {link.name}
                </Link>
