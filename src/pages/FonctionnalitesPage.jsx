...
import { ArrowRight, ShoppingBag, Box, ClipboardList, PieChart, Share2, Rocket, CheckCircle, Smartphone, Layout, Paintbrush, Filter, BarChart3, ShieldCheck, Grid, Search, Users, ArrowUpRight, Plus } from 'lucide-react';
...
const BuilderMockup = () => (
  <div className="absolute inset-0 bg-slate-100 dark:bg-slate-900 rounded-2xl overflow-hidden flex flex-col shadow-sm border border-slate-200 dark:border-slate-800">
    <div className="h-10 bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 flex items-center px-4 justify-between shrink-0">
       ...
       <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Éditeur Visuel</div>
    </div>
    <div className="flex flex-1 overflow-hidden">
       {/* Sidebar */}
       <div className="hidden sm:flex w-1/3 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 p-3 flex-col gap-3">
          <div className="text-[8px] font-black uppercase text-slate-400">Sections</div>
          {['Hero Banner', 'Grille Produits', 'Témoignages'].map((v, i) => (
             <div key={i} className="p-2 border border-slate-100 dark:border-slate-800 rounded bg-slate-50 dark:bg-slate-900 flex items-center gap-2 cursor-move hover:border-primary/50 transition-colors group">
                ...
                <span className="text-[8px] font-bold text-slate-600 dark:text-slate-300">{v}</span>
             </div>
          ))}
       </div>
       {/* Canvas */}
       <div className="flex-1 p-4 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col gap-3 overflow-y-auto">
          <div className="border-2 border-dashed border-primary/30 rounded-xl p-4 bg-primary/5 flex flex-col items-center justify-center relative group hover:border-primary transition-colors cursor-pointer min-h-[140px]">
             <div className="absolute top-2 right-2 bg-primary text-white text-[8px] font-black uppercase px-2 py-0.5 rounded opacity-0 group-hover:opacity-100 transition-opacity">Modifier</div>
    ...
    <div className="absolute inset-0 bg-white dark:bg-slate-950 rounded-2xl overflow-hidden flex flex-col shadow-sm border border-slate-200 dark:border-slate-800">
       <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center shrink-0">
          <div className="flex gap-4">
             <button onClick={() => setActiveTab('all')} className={`text-[10px] font-black uppercase tracking-widest ${activeTab === 'all' ? 'text-primary' : 'text-slate-400'}`}>Produits</button>
             <button onClick={() => setActiveTab('active')} className={`text-[10px] font-black uppercase tracking-widest ${activeTab === 'active' ? 'text-emerald-500' : 'text-slate-400'}`}>Actifs</button>
          </div>
          ...
          {[1,2,3].map((v) => (
             <div key={v} className="flex items-center justify-between p-3 rounded-xl border border-slate-100 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-900/50 hover:bg-white dark:hover:bg-slate-900 hover:shadow-md transition-all group cursor-pointer">
                ...
                <div className="flex items-center gap-4">
                   <span className="text-[10px] font-black text-slate-900 dark:text-white group-hover:text-primary transition-colors">4,500 DA</span>
          ...
          {['Confirmé', 'En attente', 'Expédié'].map((status, i) => (
             <div key={i} className="bg-white dark:bg-slate-900 rounded-xl p-3 sm:p-4 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md hover:border-primary/30 transition-all cursor-pointer flex justify-between items-center group">
                 ...
                 <span className={`text-[8px] font-black uppercase px-2.5 py-1 rounded-full whitespace-nowrap ${
                    status === 'Confirmé' ? 'bg-blue-100 text-blue-600 dark:bg-blue-500/20 dark:text-blue-400' :
...
const AnalyticsMockup = () => (
    <div className="absolute inset-0 bg-white dark:bg-slate-950 rounded-2xl overflow-hidden shadow-sm border border-slate-200 dark:border-slate-800 p-5 sm:p-6 flex flex-col group">
       <div className="flex justify-between items-start mb-6">
          <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Revenus Nette</p>
              <h4 className="text-3xl font-black text-slate-900 dark:text-white tabular-nums tracking-tighter">124,500 <span className="text-[10px] tracking-normal">DA</span></h4>
          </div>
  ...
  return (
    <div className="absolute inset-0 rounded-2xl overflow-hidden bg-slate-950 dark:bg-slate-950 flex flex-col">
      ...
      <div className="h-9 shrink-0 bg-slate-900 border-b border-slate-800 flex items-center px-4 gap-1.5">
        ...
        <div className="w-2.5 h-2.5 rounded-full bg-slate-700" />
        <div className="w-2.5 h-2.5 rounded-full bg-slate-700" />
        <span className="ml-auto text-[9px] font-bold text-slate-500 uppercase tracking-widest">Écosystème RiseGroup</span>
      </div>

      {/* Content: 2x2 grid + center hub overlay */}
      <div className="flex-1 relative flex items-center justify-center p-4 sm:p-6">
        ...
        <div className="w-full grid grid-cols-2 gap-y-4 sm:gap-y-6 relative z-0">
          {nodes.map((node, i) => (
            <div key={i} className={`flex flex-col items-center gap-1.5 group/node cursor-pointer ${i % 2 === 0 ? 'justify-self-start pl-2 sm:pl-4' : 'justify-self-end pr-2 sm:pr-4'}`}>
...
const SettingsMockup = () => (
    <div className="absolute inset-0 bg-white dark:bg-slate-950 rounded-2xl overflow-hidden flex shadow-sm border border-slate-200 dark:border-slate-800">
       <div className="hidden sm:flex w-1/3 bg-slate-50 dark:bg-slate-900 border-r border-slate-100 dark:border-slate-800 p-4 flex-col gap-3">
       ...
       <div className="flex-1 p-5 sm:p-6 flex flex-col">
           <div className="flex items-center justify-between mb-6">
              <p className="text-[10px] sm:text-xs font-black uppercase text-slate-400 tracking-[0.2em]">Permissions Système</p>
...
// Fallback icon import replacement
        ...
        <ul className="space-y-4 pt-4">
          {benefits.map((benefit, idx) => (
            <li key={idx} className="flex items-start gap-3 group/item">
              ...
              <span className="text-slate-700 dark:text-slate-300 font-bold transition-colors group-hover/item:text-slate-900 dark:group-hover/item:text-white">{benefit}</span>
            </li>
          ))}
        </ul>
      </div>
            ...
            {mockup ? (
               <div className="absolute inset-4 sm:inset-8 transition-transform duration-700 hover:rotate-x-2 hover:-rotate-y-2 hover:scale-[1.02] transform-style-3d">
                ...
                <div className="w-full bg-white dark:bg-slate-950 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 h-full flex flex-col items-center justify-center relative z-10 overflow-hidden hover:scale-[1.02] transition-all duration-500 hover:border-primary/30">
                    <Icon size={48} sm:size={64} className="text-slate-200 dark:text-slate-800 mb-4 transition-transform hover:scale-110 duration-500" />
                    <p className="text-slate-400 dark:text-slate-600 font-black text-[10px] sm:text-xs uppercase tracking-[0.2em] text-center px-4">Interface <br/>{title}</p>
                </div>
            )}
        </div>
      </div>
    ...
    <div className="bg-white dark:bg-slate-950 min-h-screen">
      ...
      <section className="pt-40 pb-20 lg:pt-48 lg:pb-32 relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 text-center max-w-4xl animate-slide-up">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800/30 mb-8 mx-auto hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors cursor-pointer">
            ...
            <span className="text-[11px] font-bold text-blue-600 dark:text-blue-400 tracking-tight">Outils E-commerce Professionnels</span>
          </div>
          <h1 className="text-5xl lg:text-7xl font-black text-slate-900 dark:text-white tracking-tight mb-8 leading-tight">
            Tout ce qu'il vous faut. <br/><span className="text-primary italic">Sans la complexité.</span>
          </h1>
          <p className="text-lg lg:text-2xl text-slate-500 dark:text-slate-400 font-medium mb-12">
            Découvrez comment chaque module de RiseCart a été pensé pour maximiser vos ventes, optimiser votre temps et structurer votre activité.
          </p>
        </div>
      </section>
      ...
      <section className="py-20 lg:py-28 border-y border-slate-100 dark:border-slate-800/60 bg-white dark:bg-slate-950 transition-colors duration-500">
        <div className="container mx-auto px-6">
          <div className="text-center mb-16">
            <p className="text-[10px] font-black text-primary uppercase tracking-[0.3em] mb-4">6 Modules Puissants</p>
            <h2 className="text-3xl lg:text-5xl font-black text-slate-900 dark:text-white tracking-tight">Tout ce dont vous avez besoin,<br/><span className="text-slate-400 dark:text-slate-500 font-medium">dans une seule plateforme.</span></h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 lg:gap-6">
            {[
              {
                num: '01', title: 'Création Boutique', icon: Layout, color: 'blue',
                desc: 'Un éditeur visuel puissant pour construire votre vitrine en quelques clics.'
              },
              {
                num: '02', title: 'Catalogue Produits', icon: Box, color: 'violet',
                desc: 'Gérez vos références, variantes et stocks depuis une interface centralisée.'
              },
              {
                num: '03', title: 'Gestion Commandes', icon: ClipboardList, color: 'emerald',
                desc: 'Traitez, expédiez et suivez chaque commande sans effort.'
              },
              {
                num: '04', title: 'Analytics & Revenus', icon: BarChart3, color: 'amber',
                desc: 'Visualisez vos chiffres clés et prenez des décisions basées sur des données réelles.'
              },
              {
                num: '05', title: 'Écosystème Natif', icon: Share2, color: 'rose',
                desc: 'Connexions natives avec RiseConfirm, RiseManager, et RisePay.'
              },
              {
                num: '06', title: 'Contrôle Avancé', icon: Rocket, color: 'indigo',
                desc: 'Permissions, rôles, logs, et infrastructure dédiée pour les vendeurs ambitieux.'
              }
            ].map((mod, i) => {
              const colorMap = {
      ...
      <section className="py-12 lg:py-24">
        <div className="container mx-auto px-6 max-w-7xl">
          <FeatureBlock
            title="Création de boutique ultra-rapide"
            description="L'apparence de votre boutique est la première impression que vous donnez. Avec notre constructeur intelligent, passez d'une idée à une boutique en ligne fonctionnelle en quelques minutes seulement."
            benefits={[
              "Modules pensés spécifiquement pour la conversion",
              "Interface Drag & Drop simple et réactive",
              "Expérience Mobile First native",
              "Sélection de thèmes modernes sans code requis"
            ]}
            icon={Paintbrush}
            mockup={<BuilderMockup />}
          />
          <FeatureBlock
            title="Gestion experte du catalogue"
            description="Fini le bricolage sur Excel. Vos produits sont votre richesse, gérez-les avec une interface centralisée conçue pour les inventaires de professionnels."
            benefits={[
              "Création illimitée de variantes (tailles, couleurs)",
              "Optimisation facile du SEO par produit",
              "Catégorisation intelligente et filtres",
              "Application rapide de réductions et promotions"
            ]}
            icon={Box}
            reversed
          ...
          <div className="my-32 bg-slate-900 dark:bg-slate-900 rounded-[3rem] p-12 lg:p-24 text-center relative overflow-hidden shadow-2xl border border-slate-800 hover:shadow-primary/10 transition-shadow duration-500 group text-reveal-container">
            ...
            <h3 className="text-3xl lg:text-5xl font-black text-white mb-6 relative z-10 tracking-tight">Prêt à transformer votre gestion ?</h3>
            <p className="text-slate-400 text-lg mb-10 max-w-2xl mx-auto relative z-10 font-medium">Démarrez maintenant avec une base solide conçue pour la vraie performance e-commerce.</p>
          ...
          <FeatureBlock
            title="Traitement logistique centralisé"
            description="Une ligne d'assemblage virtuelle pour vos commandes. Ne perdez plus la trace d'un seul colis grâce à un suivi clair et structuré."
            benefits={[
              "Changement de statuts en masse pour gagner du temps",
              "Impression instantanée des bons et factures",
              "Recherche poussée avec filtres multicritères",
              "Avertissements automatisés sur les doublons ou anomalies"
            ]}
            icon={ClipboardList}
            mockup={<OrdersMockup />}
          />
          <FeatureBlock
            ...
            description="Prenez vos décisions en fonction des chiffres, et non plus au hasard. Obtenez une vision cristalline de votre santé financière."
            benefits={[
              "Aperçu direct du Chiffre d'Affaires net",
              "Analyse instantanée du taux de conversion et panier moyen",
              "Identification de vos meilleurs produits",
              "Visualisation graphique claire de votre évolution"
            ]}
            icon={BarChart3}
            reversed
          ...
          />
          <FeatureBlock
            title="Écosystème natif RiseGroup"
            description="RiseCart s'insère parfaitement dans notre galaxie technologique. Fini les intégrations douteuses : tout communique harmonieusement."
            benefits={[
              "Connexion directe avec RiseConfirm pour le Call Center",
              "Remontée de statistiques globales vers RiseManager",
              "Pas besoin d'API complexes, tout est natif",
              "Paiements simplifiés avec RisePay (à venir)"
            ]}
            icon={Share2}
            mockup={<EcosystemMockup />}
          />
          <FeatureBlock
            title="Contrôle Premium (Plans structurés)"
            description="Quand votre activité explose, vous avez besoin de déléguer en toute sécurité. Les plans RiseCart avancés vous encadrent parfaitement."
            benefits={[
              "Ajout de collaborateurs avec permissions restreintes",
              "Sécurisation des données clients contre l'export abusif",
              "Historique des actions (logs) pour identifier qui a fait quoi",
              "Serveurs dédiés isolés pour supporter les pics massifs"
            ]}
            icon={Rocket}
            reversed
