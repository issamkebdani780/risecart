...
const products = [
  {
    id: 'risemanager',
    name: 'RiseManager',
    tagline: 'Le cerveau de votre business.',
    description: 'Pilotez votre activité commerciale en temps réel. RiseManager centralise toutes vos statistiques, vos équipes et vos performances dans un tableau de bord unifié.',
    logo: '/ecosystem/risemanager.png',
    icon: BarChart3,
    status: 'Disponible',
    statusColor: 'emerald',
    category: 'Gestion & Pilotage',
    connection: 'Remonte les revenus, commandes et analytics RiseCart en temps réel.',
    benefits: [
      'Dashboard multi-boutiques centralisé',
  ...
  },
  {
    id: 'riseconfirm',
    name: 'RiseConfirm',
    tagline: 'La confirmation devient automatique.',
    description: 'Centre d\'appels intelligent et automatisé. RiseConfirm transforme vos agents en super-confirmeurs avec des outils de gestion d\'appels, de scripts dynamiques et de relance automatique.',
    logo: '/ecosystem/RiseConfirm.jpg',
    icon: PhoneCall,
    status: 'Disponible',
    statusColor: 'emerald',
    category: 'Call Center & Confirmation',
    connection: 'Récupère automatiquement les commandes non confirmées depuis RiseCart.',
    benefits: [
      'File d\'appels automatisée depuis les commandes',
      'Scripts de confirmation dynamiques',
      'Suivi des agents et performances',
      'Relances SMS / WhatsApp intégrées',
    ],
  ...
  },
  {
    id: 'fbr',
    name: 'FBR',
    tagline: 'Livrez vite, livrez bien.',
    description: 'Solution de livraison optimisée pour le commerce en Algérie. FBR connecte votre boutique aux meilleurs transporteurs locaux avec un suivi en temps réel.',
    logo: '/ecosystem/fbr.jpg',
    icon: ShoppingCart,
    status: 'Intégré',
    statusColor: 'blue',
    category: 'Livraison & Logistique',
    connection: 'Génère les bons de livraison et met à jour les statuts directement dans RiseCart.',
    benefits: [
      'Connexion aux transporteurs locaux',
      'Génération automatique des bordereaux',
      'Suivi colis en temps réel',
      'Retours simplifiés et traçés',
    ],
  ...
  },
  {
    id: 'risepay',
    name: 'RisePay',
    tagline: 'Encaissez sans friction.',
    description: 'La solution de paiement conçue pour le e-commerce algérien. Paiements à la livraison, via CCP, ou mobile money — tout depuis une intégration native sans API complexe.',
    logo: '/ecosystem/risepay.jpg',
    icon: CreditCard,
    status: 'Bientôt',
    statusColor: 'amber',
    category: 'Paiements & Finance',
    connection: 'Réconciliation automatique des paiements avec chaque commande RiseCart.',
    benefits: [
      'Paiement à la livraison tracé',
      'CCP et mobile money natifs',
  ...
  },
  {
    id: 'riseacademy',
    name: 'RiseAcademy',
    tagline: 'Apprenez à vendre mieux.',
    description: 'La formation e-commerce pensée pour les vendeurs algériens. Cours, coaching, et ressources pratiques pour accélérer votre croissance à chaque étape.',
    logo: '/ecosystem/riseacademy.png',
    icon: GraduationCap,
    status: 'Bientôt',
    statusColor: 'rose',
    category: 'Formation & Croissance',
    connection: 'Recommandations personnalisées basées sur vos performances RiseCart.',
    benefits: [
      'Cours vidéo & tutoriels pratiques',
      'Coaching personnalisé',
      'Communauté de vendeurs actifs',
      'Stratégies testées & validées',
    ],
...
const ProductCard = ({ product, active, onClick }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full flex items-center gap-4 px-4 py-4 rounded-2xl border text-left transition-all duration-300 cursor-pointer ${
        active
    ...
    >
      ...
      <div className={`flex items-center gap-1.5 px-2 py-1 rounded-full border text-[9px] font-black uppercase tracking-wider shrink-0 ${sc.bg} ${sc.border} ${sc.text}`}>
        <div className={`w-1.5 h-1.5 rounded-full ${sc.dot} ${product.status === 'Disponible' ? 'animate-pulse' : ''}`} />
        {product.status}
      </div>
    </button>
  );
  ...
  return (
    <div className={`h-full flex flex-col rounded-[2.5rem] bg-gradient-to-br ${product.accent} dark:from-slate-900 dark:to-slate-900 border border-slate-200 dark:border-slate-800 overflow-hidden transition-all duration-500`}>
      ...
      <div className="p-8 sm:p-10 flex flex-col gap-6 flex-1">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-[10px] font-black uppercase tracking-wider ${sc.bg} ${sc.border} ${sc.text}`}>
            <div className={`w-2 h-2 rounded-full ${sc.dot} ${product.status === 'Disponible' ? 'animate-pulse' : ''}`} />
            {product.status}
          </div>
        </div>
        ...
        <div className="bg-primary/5 dark:bg-primary/10 border border-primary/20 dark:border-primary/20 rounded-2xl p-4 flex items-start gap-3">
          ...
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-primary mb-1">Connexion avec RiseCart</p>
        ...
        {/* Benefits */}
        <div>
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4">Ce que vous gagnez</p>
      ...
      <div className="px-8 sm:px-10 pb-8 pt-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between gap-4">
        <span className="text-xs text-slate-400 font-medium">Partie intégrante de l'écosystème RiseGroup</span>
        {product.status !== 'Bientôt' ? (
          ...
          <span className="text-xs font-black text-amber-500 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 px-4 py-2 rounded-xl">Bientôt disponible</span>
        )}
      </div>
    </div>
  );
      ...
      <section className="pt-40 pb-20 lg:pt-48 lg:pb-28 relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 text-center max-w-4xl">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800/30 mb-8 mx-auto">
            ...
            <span className="text-[11px] font-bold text-blue-600 dark:text-blue-400">5 outils · 1 galaxie cohérente</span>
          </div>
          <h1 className="text-5xl lg:text-7xl font-black text-slate-900 dark:text-white tracking-tight mb-6 leading-tight">
            RiseCart ne travaille<br/><span className="text-primary italic">jamais seul.</span>
          </h1>
          <p className="text-lg lg:text-xl text-slate-500 dark:text-slate-400 font-medium mb-0 max-w-2xl mx-auto">
            Chaque produit du RiseGroup a été conçu pour s'intégrer nativement avec RiseCart. Pas d'API douteuse, pas d'export manuel — tout communique en temps réel.
          </p>
        </div>
      </section>
        ...
        <div className="container mx-auto px-6">
              ...
              {[
                ...
                return (
                  <button
                    ...
                    className={`absolute w-14 h-14 rounded-2xl overflow-hidden border-2 transition-all duration-300 z-10 bg-white dark:bg-slate-900 shadow-lg ${
                      activeId === p.id
          ...
          <div className="grid lg:grid-cols-[300px_1fr] xl:grid-cols-[340px_1fr] gap-6 lg:gap-8 max-w-6xl mx-auto">
            ...
            <div className="bg-slate-50/50 dark:bg-slate-900/30 rounded-3xl border border-slate-100 dark:border-slate-800 p-3 flex flex-col gap-1 lg:self-start lg:sticky lg:top-28">
              <p className="text-[9px] font-black uppercase tracking-[0.25em] text-slate-400 px-4 pt-2 pb-3">Modules</p>
              {products.map(p => (
              ...
              <div className="flex items-center gap-3 px-4 py-3 mt-2 rounded-2xl border border-dashed border-slate-200 dark:border-slate-700">
                ...
                <span className="text-xs font-bold text-slate-400">Et plus à venir…</span>
              </div>
            </div>
      ...
      <section className="py-20 lg:py-28 bg-slate-50/50 dark:bg-slate-900/30 border-y border-slate-100 dark:border-slate-800">
        <div className="container mx-auto px-6">
          <div className="text-center mb-16 max-w-3xl mx-auto">
            <p className="text-[10px] font-black text-primary uppercase tracking-[0.3em] mb-4">Logique de continuité</p>
            <h2 className="text-3xl lg:text-5xl font-black text-slate-900 dark:text-white tracking-tight">Le parcours d'une<br/><span className="text-slate-400 dark:text-slate-500 font-medium">commande de A à Z.</span></h2>
          </div>
          <div className="relative max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-4">
              {[
                { step: '01', label: 'Commande créée', tool: 'RiseCart', color: 'blue',   desc: 'Le client passe sa commande sur votre boutique RiseCart.' },
                { step: '02', label: 'Appel de confirmation', tool: 'RiseConfirm', color: 'emerald', desc: 'RiseConfirm alerte l\'agent et génère le script d\'appel adapté.' },
                { step: '03', label: 'Expédition lancée', tool: 'FBR / Livreur', color: 'violet', desc: 'Le bon de livraison est généré et transmis au transporteur.' },
                { step: '04', label: 'Revenus tracés', tool: 'RiseManager', color: 'amber', desc: 'Les statistiques remontent en temps réel dans votre dashboard.' },
              ].map((s, i) => (
                <div key={i} className="relative flex flex-col items-center text-center group cursor-default">
                  <div className={`w-20 h-20 rounded-[1.5rem] mb-5 flex items-center justify-center text-2xl font-black relative z-10 shadow-xl transition-transform duration-300 group-hover:-translate-y-1 ${
                    s.color === 'blue'    ? 'bg-blue-500 text-white shadow-blue-200 dark:shadow-blue-500/20' :
